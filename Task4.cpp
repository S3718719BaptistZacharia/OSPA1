...
void map4(string cleanFileName){
    // split the all words lengths in the string array to their own file
    cout << "*** Map4() slitting data on word lengths ***\n";
    ...
    if (id > 0){
        ...
        for (int i = MINCOUNT; i <= MAXCOUNT; i++){
            string commandStr = "sort -k1.3,1.15 T4words"+ to_string(i) + ".txt > T4words"
                                + to_string(i) + "sorted.txt";

            const char * command = commandStr.c_str();
            // open a pipe in write mode (w) to run the command above. exit when error occurs
            if ((pipe_open = popen(command, "w")) == NULL){
                perror("popen error in map4(). sort on 3rd letter");
                exit(1);
            }
        }
        // close pipe
...
void reduce4(){
    ...
    for (int i = MINCOUNT; i <= MAXCOUNT; i++){
        commandStr += "T4words" + to_string(i) + "sorted.txt ";
    }
    // open a pipe in write mode (w) to run the command above. exit when error occurs
    commandStr += "| sort > T4mergedFile.txt";
    const char * command = commandStr.c_str();
    if ((pipe_open = popen(command, "w")) == NULL){
